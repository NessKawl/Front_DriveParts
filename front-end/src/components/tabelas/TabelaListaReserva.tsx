...
import TabelaLinhaReserva from "./TabelaLinhaReserva";
...
export default function TabelaListaReserva({
  titulo,
  colunas,
  fetchData,
  ...
  return (
    <div className="bg-black-smooth border-l border-primary-orange p-2 w-full h-full">
      ...
      {loading ? (
        ...
        >
          <ul className="divide-y divide-gray-200/40 divide-dashed">
            ...
            {dados.map((item, i) => (
              <><TabelaLinhaReserva key={i} colunas={colunas} item={item} /></>
            ))}
          </ul>
        </div>
      )}
