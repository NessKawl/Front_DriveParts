...
export default function Catalogo() {
  ...
  const productsGridMaisVendidos = produtos.slice(0, 6).map((p) => ({
  ...
  const productsGridDestaque = produtos.slice(6, 12).map((p) => ({
    image: p.pro_caminho_img || "/sem-imagem.jpg",
    name: p.pro_nome,
    id: p.pro_aux_uuid,
    price: `R$ ${p.pro_valor.toFixed(2).replace(".", ",")}`,
